  ...
  return (
    <footer className="border-t border-[#2A2A38] bg-[#0A0A0F]">
      <div className="mx-auto flex max-w-[1100px] flex-col gap-10 px-6 py-12 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex flex-col gap-3">
          <h4 className="font-sans text-sm font-semibold text-[#EEEEFF]">
            Quick Links
          </h4>
          <nav className="flex flex-col gap-2">
            {navLinks.map(({ href, label }) => (
              <Link
                key={href}
                href={href}
                className="font-sans text-sm text-[#7A7A9A] transition-all duration-200 hover:text-[#EEEEFF]"
              >
                {label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-4">
            {socialLinks.map(({ href, label, icon: Icon }) => (
              <a
                ...
                aria-label={label}
                className="text-[#7A7A9A] transition-all duration-200 hover:text-[#6C63FF]"
              >
                <Icon />
              </a>
            ))}
